...
def _get_params_builtin_fn(func: Callable) -> Optional[List[inspect.Parameter]]:
    if (
...
def _get_params_ufunc(func: Callable) -> Optional[List[inspect.Parameter]]:
...
def _get_params(func: Callable) -> Dict[str, inspect.Parameter]:
    ...
    try:
        params = (
            list(get_signature(func).parameters.values())
            or _get_params_ufunc(func)
